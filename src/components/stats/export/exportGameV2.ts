...
export const exportJSON = (data: GameStatsData) => {
  const payload = {
    ...
    // The full post-game package — ONE engine (gameReport.ts) produces the
    // analytics, per-player packages, hexbins, and auto-derived highlights.
    report: buildGameReport(data.gameData, data.shots, data.actions),
  };
