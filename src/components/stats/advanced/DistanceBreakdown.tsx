// src/components/stats/advanced/DistanceBreakdown.tsx
// FG% by distance band — horizontal bars. Read-only on shot_events.

import { useMemo, useState } from 'react';
import type { ShotEvent } from '../../shotchart/types/shotTypes';
import type { GameBoxScore, TeamSide } from '../types';
import { distanceBands } from '../../../services/statsEngine';
import { Card } from '../ui/Card';
import { SectionHeader } from '../ui/SectionHeader';
import { cx } from '../ui/cx';

export const DistanceBreakdown = ({ box, shots }: { box: GameBoxScore; shots: ShotEvent[] }) => {
  const [side, setSide] = useState<TeamSide>('A');
  const bands = useMemo(() => distanceBands(shots, side), [shots, side]);
      ...
      <Card padded>
        <div className="space-y-3">
          {bands.every(b => b.fga === 0) ? (
            <p className="text-sm text-slate-400 dark:text-zinc-500 text-center py-6">No located shots for this team.</p>
          ) : (
            bands.map(b => (
              <div key={b.label}>
                <div className="flex items-center justify-between mb-1 text-xs">
                  ...
                  <span className="tabular-nums text-slate-500 dark:text-zinc-400">
                    {hasMisses ? (
                      ...
                      <><span className="font-bold text-slate-900 dark:text-zinc-100">{b.fgm}</span> made</>
                    )}
                  </span>
                </div>
                <div className="h-2.5 rounded-full bg-slate-100 dark:bg-zinc-800 overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${hasMisses ? Math.min(100, b.fgPct) : (b.fga ? 100 : 0)}%`,
                      backgroundColor: accent,
                      opacity: hasMisses ? 1 : 0.5,
                    }}
                  />
                </div>
              </div>
