...
const DynamicCourtCard = ({ gameCode }: { gameCode: string }) => {
    ...
    return (
        <div className="bg-white border border-slate-200 dark:bg-black dark:border-zinc-800 rounded-lg overflow-hidden flex flex-col shadow-2xl relative group">
    ...
    if (gameCodes.length === 0) {
        return (
            <div className="min-h-screen bg-white dark:bg-black flex flex-col items-center justify-center text-slate-400 dark:text-zinc-500 font-mono text-sm gap-4">
                <div className="text-slate-300 dark:text-zinc-700 text-4xl font-black italic uppercase">Box Arena</div>
                <div className="animate-pulse">WAITING FOR COURT CONFIGURATION...</div>
                <div className="text-xs text-slate-400 dark:text-zinc-600 mt-4">Add ?games=CODE1,CODE2 to the URL</div>
            </div>
        );
    }
