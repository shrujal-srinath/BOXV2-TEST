...
//  Theme-aware: light = white/red/black, dark = black/zinc (LED wall)
// ═══════════════════════════════════════════════════════════════
...
const LIGHT: ArenaColors = {
    pageBg:       '#ffffff',
    cardBg:       '#ffffff',
    cardBorder:   '#e2e8f0',
    labelBarBg:   '#f8fafc',
    labelBarBorder:'#e2e8f0',
    courtLabel:   '#334155',
    clockText:    '#64748b',
    scoreTxt:     '#0f172a',
    vsTxt:        '#94a3b8',
...
const DARK: ArenaColors = {
    pageBg:       '#000000',
    cardBg:       '#050505',
    cardBorder:   '#111111',
    labelBarBg:   '#0a0a0a',
    labelBarBorder:'#1a1a1a',
    courtLabel:   '#444444',
    clockText:    '#333333',
    scoreTxt:     '#ffffff',
    vsTxt:        '#2a2a2a',
    ...
    headerMeta:   '#2a2a2a',
    headerRight:  '#1e1e1e',
    spinnerBorder:'#1a1a1a',
    notFoundCode: '#1a1a1a',
    notFoundHint: '#222222',
};

// ─── Dynamic CSS (background must match theme to avoid flicker) ───────────────
...
const getArenaStyles = (isLight: boolean) => `
  ...
  html, body, #root { margin: 0; padding: 0; background: ${isLight ? '#ffffff' : '#000000'}; overflow: hidden; }
...
const ArenaScoreCard: React.FC<{
    gameCode: string;
    courtNumber: number;
    totalCourts: number;
    c: ArenaColors;
    isLight: boolean;
}> = ({ gameCode, courtNumber, totalCourts, c, isLight }) => {
    ...
    // In light mode, reduce glow effects so they work on white
    const scoreGlowA = isLight
        ? (pulseA ? `0 2px 12px ${live.teamAColor}44` : 'none')
        : (pulseA ? `0 0 40px ${live.teamAColor}, 0 0 80px ${live.teamAColor}66` : `0 0 20px ${live.teamAColor}22`);
    const scoreGlowB = isLight
        ? (pulseB ? `0 2px 12px ${live.teamBColor}44` : 'none')
        : (pulseB ? `0 0 40px ${live.teamBColor}, 0 0 80px ${live.teamBColor}66` : `0 0 20px ${live.teamBColor}22`);
        ...
        <div style={{
            ...
            <div style={{ flex: 1, display: 'flex', alignItems: 'stretch', minHeight: 0 }}>
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: totalCourts <= 2 ? '1.5rem' : '0.75rem', position: 'relative' }}>
                    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, background: `radial-gradient(ellipse at 20% 50%, ${live.teamAColor}08 0%, transparent 70%)`, pointerEvents: 'none' }} />
                    <div style={{ fontFamily: 'Oswald, monospace', fontWeight: 700, fontSize: teamFontSize, color: live.teamAColor, textTransform: 'uppercase', letterSpacing: '0.05em', textAlign: 'center', marginBottom: '0.4rem', lineHeight: 1.1 }}>
                        {live.teamAName}
                    </div>
                    <div style={{
                ...
                {/* Team B */}
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: totalCourts <= 2 ? '1.5rem' : '0.75rem', position: 'relative' }}>
                    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, background: `radial-gradient(ellipse at 80% 50%, ${live.teamBColor}08 0%, transparent 70%)`, pointerEvents: 'none' }} />
                    <div style={{ fontFamily: 'Oswald, monospace', fontWeight: 700, fontSize: teamFontSize, color: live.teamBColor, textTransform: 'uppercase', letterSpacing: '0.05em', textAlign: 'center', marginBottom: '0.4rem', lineHeight: 1.1 }}>
                        {live.teamBName}
                    </div>
                    <div style={{
                        fontFamily: 'Oswald, monospace', fontWeight: 900, fontSize: scoreSize,
                        color: c.scoreTxt, lineHeight: 0.9, fontVariantNumeric: 'tabular-nums',
                        textShadow: scoreGlowB,
...
const PendingCourtTile: React.FC<{ courtNumber: number; request: PendingRequest; c: ArenaColors }> = ({ courtNumber, request, c }) => (
    <div style={{
        ...
        <div style={{ width: 2, height: '60%', background: `linear-gradient(to bottom, transparent, ${c.emptyBorder}, transparent)`, position: 'absolute', left: '50%' }} />
...
const IdleScreen: React.FC<{ arenaCode: string; label?: string; c: ArenaColors; arenaStyles: string }> = ({ arenaCode, label, c, arenaStyles }) => (
    <div style={{ width: '100vw', height: '100vh', background: c.pageBg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '2rem', fontFamily: 'Oswald, monospace' }}>
        ...
        <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ fontSize: '5rem', fontWeight: 900, letterSpacing: '0.5em', color: c.idleCodeTxt, fontVariantNumeric: 'tabular-nums', padding: '0 1rem' }}>
                {arenaCode}
            </div>
        </div>
...
const LoadingScreen: React.FC<{ c: ArenaColors; arenaStyles: string }> = ({ c, arenaStyles }) => (
...
const NotFoundScreen: React.FC<{ arenaCode: string; c: ArenaColors; arenaStyles: string }> = ({ arenaCode, c, arenaStyles }) => (
    <div style={{ width: '100vw', height: '100vh', background: c.pageBg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1rem', fontFamily: 'Oswald, monospace' }}>
        ...
        <div style={{ fontSize: '3rem', fontWeight: 900, letterSpacing: '0.4em', color: c.notFoundCode }}>{arenaCode}</div>
        <div style={{ fontSize: '0.55rem', letterSpacing: '0.2em', color: c.notFoundHint }}>Check the arena code and try again</div>
    </div>
);
...
export const ArenaView: React.FC = () => {
    ...
    const arenaStyles = getArenaStyles(isLight);
    ...
    if (loading) return <LoadingScreen c={c} arenaStyles={arenaStyles} />;
    if (notFound || !session) return <NotFoundScreen arenaCode={code} c={c} arenaStyles={arenaStyles} />;
    ...
    if (activeCodes.length === 0 && pendingRequests.length === 0) {
        return <IdleScreen arenaCode={code} label={session.label} c={c} arenaStyles={arenaStyles} />;
    }
        ...
        <>
            ...
            <div style={{ width: '100vw', height: '100vh', background: c.pageBg, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ background: c.headerBg, borderBottom: `1px solid ${c.headerBorder}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 1.5rem', height: 36, flexShrink: 0 }}>
                    ...
                    <div style={{ fontFamily: 'Oswald, monospace', fontWeight: 400, fontSize: '0.55rem', letterSpacing: '0.3em', color: c.headerMeta }}>
                        ARENA {code} · {session.label.toUpperCase()} · {activeCodes.length}/{totalSlots} COURTS LIVE
                    </div>
                    <div style={{ fontFamily: 'Oswald, monospace', fontWeight: 400, fontSize: '0.55rem', letterSpacing: '0.2em', color: c.headerRight }}>
                        MULTI-COURT DISPLAY
                    </div>
                </div>
