...
export const LandingPage: React.FC = () => {
  ...
  useEffect(() => {
    const unsubAuth = subscribeToAuth((u) => {
      if (u && !showSplash) navigate('/dashboard');
    });
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-black font-sans text-slate-900 dark:text-white flex flex-col relative overflow-x-hidden animate-in">

      {/* HEADER */}
      <header className="flex justify-between items-center p-6 border-b border-slate-200 dark:border-zinc-900 bg-white/85 dark:bg-black/80 backdrop-blur-md z-40 sticky top-0 shadow-sm dark:shadow-none">
        <div className="flex items-center gap-4">
          <div className="relative w-10 h-10 flex items-center justify-center">
            <svg className="w-8 h-8 text-red-600 drop-shadow-[0_0_8px_rgba(220,38,38,0.4)]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
          ...
          <div className="flex flex-col justify-center">
            <h1 className="text-2xl font-black italic tracking-tighter leading-none text-slate-900 dark:text-white">THE BOX</h1>
            <p className="text-[9px] text-slate-400 dark:text-zinc-500 font-bold uppercase tracking-[0.3em] leading-none mt-1">By BMSCE</p>
          </div>
        </div>

        <div className="flex items-center gap-4">
          <div className="hidden md:flex items-center gap-2 border border-slate-200 dark:border-zinc-800 bg-slate-100 dark:bg-zinc-950 px-3 py-1.5 rounded-sm">
          ...
          <div className="relative">
            <button
              ...
              className="p-2 hover:bg-slate-100 dark:hover:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-sm transition-colors text-slate-500 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-white cursor-pointer"
            >
            ...
            {showUserMenu && (
              <>
                ...
                <div className="absolute right-0 mt-3 w-64 bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 shadow-xl dark:shadow-2xl z-50 animate-in">
                  <div className="p-4 border-b border-slate-100 dark:border-zinc-900">
                    <p className="text-[10px] font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-[0.2em]">System Menu</p>
                  </div>
                  <button
                    onClick={() => { setShowEmailModal(true); setShowUserMenu(false); }}
                    className="flex items-center gap-3 w-full px-4 py-4 text-left hover:bg-slate-50 dark:hover:bg-zinc-900 transition-colors cursor-pointer"
                  >
                    <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-zinc-800 flex items-center justify-center">
                      <svg className="w-4 h-4 text-slate-400 dark:text-zinc-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1" /></svg>
                    </div>
                    <span className="text-sm font-bold text-slate-700 dark:text-zinc-300">Operator Login</span>
                  </button>
                </div>
              </>
            )}
