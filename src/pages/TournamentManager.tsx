                ...
                <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
                    <div className="flex items-center gap-8">
                        <div className="flex items-center gap-4">
                            <button onClick={() => navigate('/tournament')} className="text-zinc-500 hover:text-white transition-colors">Back</button>
                    ...
                    <div className="flex items-center gap-4">
                        {selectedDivisionId ? (
                            <button onClick={() => setSelectedDivisionId(null)} className="border border-zinc-700 hover:border-white text-zinc-400 hover:text-white px-4 py-2 rounded text-[10px] font-bold uppercase tracking-widest transition-all">Close View</button>
                        ) : isAdmin ? (
                            <button onClick={() => setShowAddSportModal(true)} className="bg-white text-black px-5 py-2 rounded font-black uppercase text-[10px] tracking-widest hover:bg-zinc-200 transition-transform hover:-translate-y-0.5">+ Add Event</button>
                        ) : null}
                    </div>
                </div>
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className="flex justify-between items-start mb-6">
                                                    <h3 className={`text-2xl font-black italic uppercase tracking-tight ${div.isActive ? 'text-white' : 'text-zinc-600'}`}>{div.sport}</h3>
                                                    {isAdmin && <button onClick={(e) => handleRemoveDivision(e, div.id)} className="text-zinc-600 hover:text-red-500 p-1">&times;</button>}
                                                </div>
                                                <div className="space-y-1 mb-8">
                                                    <div className="flex items-center gap-2">
                                                        <div className={`w-2 h-2 rounded-full ${statusColor}`}></div>
                                                        <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-300">{statusText}</span>
                                                    </div>
                                                    {div.isActive && <div className="text-xs font-mono text-zinc-400">{div.bracketSize} Teams</div>}
                                                </div>
                                                <div className={`w-full py-3 text-center rounded font-black uppercase text-[10px] tracking-[0.2em] transition-colors ${div.isActive ? 'bg-white text-black group-hover:bg-yellow-500' : 'bg-zinc-800 text-zinc-500 group-hover:text-zinc-300'}`}>{buttonText}</div>
                                            </div>
                                        );
                                    })}
                                </div>
                                ...
                                <div className="flex items-center gap-4">
                                    {currentDivision.status === 'draft' && (
                                        <>
                                            ...
                                            <button onClick={handlePublish} className="bg-green-600 hover:bg-green-500 text-white px-4 py-2 rounded font-bold uppercase text-[10px] tracking-widest shadow-lg shadow-green-900/20">Publish Bracket</button>
                                        </>
                                    )}
                                    {currentDivision.status === 'published' && (
                                        <div className="flex items-center gap-4">
                                            <div className="flex items-center gap-2 border-r border-zinc-800 pr-4">
                                                <span className={`text-[9px] font-bold uppercase tracking-widest ${isEditMode ? 'text-red-500' : 'text-zinc-600'}`}>{isEditMode ? 'EDIT MODE' : 'LOCKED'}</span>
                                                <div onClick={() => setIsEditMode(!isEditMode)} className={`w-8 h-4 rounded-full cursor-pointer relative transition-colors ${isEditMode ? 'bg-red-500' : 'bg-zinc-800'}`}>
                                            ...
                                            <button onClick={handleUnpublish} className="text-zinc-700 hover:text-red-900 text-[10px] font-bold uppercase">Reset</button>
                                        </div>
                                    )}
                                </div>
                            )}
            ...
            {setupDivId && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm p-4">
                    <div className="bg-zinc-900 border border-zinc-700 p-10 rounded-2xl w-full max-w-md shadow-2xl">
                        <h2 className="text-xl font-black uppercase text-white mb-1 tracking-wide">Setup <span className="text-yellow-500">{setupDivId.split('_')[0]}</span></h2>
                        <div className="mb-8 mt-8">
                            ...
                            {/* Gold Text Update */}
                            <div className="bg-black border border-zinc-800 rounded-lg p-4 flex flex-col gap-2">
                                <div className="flex justify-between items-center text-xs font-mono text-zinc-500 uppercase">
                                    <span>System Size</span>
                                    <span className="text-yellow-500 font-bold">{bracketSize} Slots</span>
                                </div>
                                <div className="flex justify-between items-center text-xs font-mono text-zinc-500 uppercase">
                                    <span>Byes Awarded</span>
                                    <span className="text-yellow-500 font-bold">{byes} Teams</span>
                                </div>
                            </div>
                        </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                    <div className="bg-zinc-900 border border-zinc-700 p-8 rounded-2xl w-full max-w-lg shadow-2xl">
                        ...
                        <div className="grid grid-cols-2 gap-3 mb-8">
                            {addableSports.map(s => (
                                <button key={s} onClick={() => handleAddSport(s)} className="p-4 rounded bg-black border border-zinc-800 hover:border-yellow-600 hover:text-yellow-500 text-left transition-all group">
